...
class SendMessageBotApp:
    ...
    async def _create_broadcasters(self):
        ...
        # B2B Broadcaster - использует аккаунт acc1 (ID: ОПТОВЫЙ)
        ...
        # B2C Broadcaster - использует аккаунт acc2 (ID: РОЗНИЧНЫЙ)
        ...
        # print(f"✅ AAA Broadcaster создан (acc1): {len(self.config.targets)} чатов, {len(self.config.aaa_messages)} сообщений")
        ...
        print(f"✅ GUS Broadcaster создан (acc2): {len(self.config.targets)} чатов, {len(self.config.gus_messages)} сообщений")
